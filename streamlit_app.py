...
# Actualizado: Fusión con GRU (en el modelo) y ARIMA (ensemble híbrido)
...
**Novedad: Fusión con GRU y ARIMA**  
- **LSTM + GRU:** Combina capas LSTM (memoria larga) con GRU (más eficiente en cómputo) en un modelo secuencial híbrido. Funciona capturando patrones complejos en series temporales financieras, donde LSTM maneja dependencias largas y GRU acelera el entrenamiento sin perder precisión.  
- **LSTM + ARIMA (Ensemble):** Predice con LSTM (no lineal, IA) y ARIMA (estadístico, lineal), luego fusiona resultados ponderados (e.g., 70% LSTM + 30% ARIMA). Mejora robustez en mercados volátiles peruanos, combinando aprendizaje profundo con modelado tradicional para reducir errores en tendencias estacionales.
""")
...
if not st.session_state.logged_in:
    ...
    # Modo de fusión
    fusion_mode = st.sidebar.selectbox("Modo de Fusión", ["LSTM Puro", "LSTM + GRU (Híbrido RNN)", "LSTM + ARIMA (Ensemble)"])
    ...
    if st.sidebar.button("Ejecutar Modelo"):
        with st.spinner("Entrenando modelo..."):
            ...
            # Fusión con GRU si seleccionado
            if fusion_mode == "LSTM + GRU (Híbrido RNN)":
                model.add(GRU(50, return_sequences=False))  # Añade GRU después de LSTM
            else:
            ...
            for _ in range(14):
                ...
                future_preds_rnn.append(pred[0, 0])
                current = np.append(current[1:], pred, axis=0)

            future_preds_rnn = np.array(future_preds_rnn).reshape(-1, 1)

            # Fusión con ARIMA si seleccionado (ensemble)
            if fusion_mode == "LSTM + ARIMA (Ensemble)":
                # ARIMA simple en precios originales
                arima_model = ARIMA(prices, order=(5,1,0))
                arima_fit = arima_model.fit()
                arima_forecast = arima_fit.forecast(steps=14)

                # Ensemble: 70% RNN + 30% ARIMA
                future_preds_rnn_descaled = scaler.inverse_transform(future_preds_rnn)
                future_preds = 0.7 * future_preds_rnn_descaled + 0.3 * arima_forecast.reshape(-1, 1)
            else:
                future_preds = future_preds_rnn

            # 6. Ajuste con macros
            macro_impact = (tc - 3.78)*0.02 + (tasa - 5.25)*(-0.015) + (cobre - 4.35)*0.03
            future_preds = future_preds * (1 + macro_impact)

            # 7. Descalar (si no hecho)
            if fusion_mode != "LSTM + ARIMA (Ensemble)":
            ...
            # 8. Resultados
            st.success(f"Modelo {fusion_mode} entrenado y predicción generada")
            ...
            fig.add_trace(go.Scatter(x=future_dates, y=predictions.flatten(), name=f"{fusion_mode} + Macros", line=dict(color="green", dash="dash")))
            fig.update_layout(title=f"Predicción {fusion_mode} - {ticker} | Kallpa Securities SAB")
